...
LRESULT CALLBACK WndProc(HWND hWnd, UINT iMessage, WPARAM wParam, LPARAM IParam)
{
	...
	case WM_KEYDOWN:
		// 아무 키나 누르면 대화상자를 띄운다.
		DialogBox(g_hInst, MAKEINTRESOURCE(IDD_DIALOG2), hWnd, (DLGPROC)Dlg6_1Proc);
...
BOOL CALLBACK Dlg6_1Proc(HWND hDlg, UINT iMsg, WPARAM wParam, LPARAM IParam)
{
	...
	case WM_COMMAND:
		...
		case IDC_ENJOY:
			...
			{
				GetDlgItemText(hDlg, ID_NUMBER, phone, 30);
				GetDlgItemText(hDlg, IDC_MALE, str, 30);
				SendMessage(hCombo, CB_GETLBTEXT, selection, (LPARAM)birth);
				sprintf(list, "이름:%s  성별:%s  전화번호:%s 생년월일:%s", name, str, phone, birth);
		...
		case IDC_SORT:
			...
			break;
		case ID_STORE:
			fw = fopen("member.txt", "w");

			for (int j = 0; j < a; j++)
			{
				fprintf(fw, "%s", list2[j]);
				fprintf(fw, "\n");
			}
			fprintf(fw, "가입자 수 : %d", a);
